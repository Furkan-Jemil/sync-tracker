    ...
    <div className="flex h-screen overflow-hidden">
      ...
      <aside className="hidden md:flex w-16 bg-slate-900 border-r border-slate-800 flex flex-col items-center py-6 gap-6 shrink-0">
        ...
        <div className="mt-auto flex flex-col items-center gap-3">
          ...
          <div className="w-8 h-8 rounded-full bg-gradient-to-br from-emerald-400 to-teal-500 text-[10px] font-black flex items-center justify-center text-white shadow-md relative group/profile cursor-pointer">
            {user?.name?.slice(0, 2).toUpperCase() || "UN"}
            
            {/* Hover Popover (Desktop) */}
            <div className="absolute left-14 bottom-0 w-48 bg-slate-900 border border-slate-800 rounded-xl shadow-2xl p-3 opacity-0 group-hover/profile:opacity-100 pointer-events-none group-hover/profile:pointer-events-auto transition-all z-[100] translate-x-2 group-hover/profile:translate-x-0">
              <div className="flex flex-col gap-1">
                <p className="text-xs font-bold text-white truncate">{user?.name}</p>
                <p className="text-[10px] text-slate-500 truncate font-mono">{user?.email}</p>
              </div>
              <div className="absolute left-[-6px] bottom-3 w-3 h-3 bg-slate-900 border-l border-b border-slate-800 rotate-45" />
            </div>
          </div>
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        <nav className="md:hidden h-16 shrink-0 bg-slate-900 border-t border-slate-800 flex items-center justify-around px-2 z-[90]">
          ...
          <div className="w-8 h-8 rounded-full bg-gradient-to-br from-emerald-400 to-teal-500 text-[10px] font-black flex items-center justify-center text-white shadow-md relative group/profile cursor-pointer">
            ...
            {/* Hover Popover (Desktop & Mobile) */}
            <div className="absolute bottom-full right-0 mb-3 w-48 bg-slate-900 border border-slate-800 rounded-xl shadow-2xl p-3 opacity-0 group-hover/profile:opacity-100 pointer-events-none group-hover/profile:pointer-events-auto transition-all z-[100] translate-y-2 group-hover/profile:translate-y-0">
              ...
              <button
                onClick={() => logout()}
                className="w-full py-2 bg-red-500/10 hover:bg-red-500/20 text-red-400 text-[10px] font-bold rounded-lg border border-red-500/20 transition-all flex items-center justify-center gap-2"
              >
                <LogOut className="w-3.5 h-3.5" />
                TERMINATE SESSION
              </button>
              {/* Arrow */}
              <div className="absolute bottom-[-6px] right-3 w-3 h-3 bg-slate-900 border-r border-b border-slate-800 rotate-45" />
            </div>
          </div>
        </nav>
